...
def main():
    ...
    # =========================
    # CENÁRIO (Zelda-like: vale + montanhas bem nas bordas)
    # =========================
    def criar_mapa(plat1_cor, plat2_cor, ramp_cor):
        ...
        # - então colocamos "paredões" por volta de z ~= +/-16.5 e com profundidade grande
        #   para encostar e preencher as laterais (sem ficar centralizado).
        Z_WALL = 16.5
        D_WALL = -9.0   # mais profundo que antes (preenche melhor as laterais)
        H_WALL = 7.0
    ...
    # =========================
    # TRECHOS (Zelda-like: encontros no vale, ranged em platô em momentos pontuais)
    # =========================
    ZMIN, ZMAX = -8.5, 8.5   # vale (mais estreito que antes)
    trechos = [
        ...
        # aproximação do platô 2 (x ~ 11, z ~ -7)
    ...
    while not glfw.window_should_close(win):
        ...
        # =========================
        # HUD VIDA (2D fixo na tela)
        # =========================
        frac = 0.0
        if player.vida_max > 0:
            frac = max(0.0, min(1.0, player.vida / player.vida_max))

        # Desenha em "screen space" usando uma projeção ortográfica em NDC (-1..1)
        vp_hud = ortho(-1, 1, -1, 1)

        glDisable(GL_DEPTH_TEST)

        # Posição no canto superior esquerdo (NDC)
        # x: -1 (esquerda) -> 1 (direita)
        # y: -1 (baixo)    -> 1 (cima)
        HUD_X = -0.78
        HUD_Y =  0.82

        # Tamanhos em NDC (ajusta fácil)
        HUD_W = 0.55
        HUD_H = 0.06

        # Fundo (barra vazia)
        model_bg = translacao(HUD_X, HUD_Y, 0.0) @ escala(HUD_W, HUD_H, 1.0)
        desenhar(hud_bg, model_bg, vp_hud, programa)

        # Vida preenchida (ancorada à esquerda)
        # Para ancorar à esquerda: desloca metade da barra "que falta"
        fill_w = HUD_W * frac
