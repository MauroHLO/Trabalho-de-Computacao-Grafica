...
def y_spawn_seguro(x, z, plataformas, rampas, half=0.5):
    ...
    if rampas:
        for r in rampas:
            if r.contencao(x, z):
                y_base = max(y_base, float(r.altura(x, z)))

    # plataformas (topo)
    for p in plataformas:
        if p.contencao(x, z):
            y_base = max(y_base, float(p.h))
...
class Fase:
    ...
    def ativar_trecho(self, t: Trecho, mundo: int, cfg_mundo: dict, plataformas, rampas,
                      cor_inim=(0.55, 0.20, 0.20)):
        if t.ativo:
            return
        t.ativo = True
    ...
    def update(self, player, mundo: int, cfg_mundo: dict, plataformas, rampas,
               cor_inim=(0.55, 0.20, 0.20)):
        ...
        if t is not None:
            self.ativar_trecho(t, mundo, cfg_mundo, plataformas, rampas, cor_inim=cor_inim)
            for e in t.inimigos:
